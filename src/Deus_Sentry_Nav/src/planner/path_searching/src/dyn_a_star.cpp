...
double AStar::getEuclHeu(GridNodePtr node1, GridNodePtr node2)
{
    Vector3d diff = (node2->index - node1->index).cast<double>();
    // double z_weight = 99999;  
    // diff.z() *= z_weight;
    diff.z() *= 0;
    ...
    while (!openSet_.empty())
    {
        ...
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
                ...
                // int fixed_z = start_idx(2);  // 固定为起点高度
                ...
                for (int dz = 0; dz <= 0; dz++)
                {
                    ...
                    // double static_cost = sqrt(dx * dx + dy * dy + (dz * z_weight) * (dz * z_weight));
